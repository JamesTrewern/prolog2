...
fn upcase_impl(heap: &mut QueryHeap, goal: usize, string_mode: bool) -> PredReturn {
    ...
    let Some(text) = read_text(heap, in_a) else {
        return false.into();
    };
...
mod tests {
    ...
    fn atom_string_forward() {
        assert_eq!(
            tw().binding("atom_string(hello, S).", "S").as_deref(),
            Some("\"hello\"")
        );
    }
    ...
    fn atom_string_backward() {
        assert_eq!(
            tw().binding("atom_string(A, \"world\").", "A").as_deref(),
            Some("world")
        );
    }
    ...
    fn atom_number_forward() {
        assert_eq!(
            tw().binding("atom_number('42', N).", "N").as_deref(),
            Some("42")
        );
        assert_eq!(
            tw().binding("atom_number('3.14', N).", "N").as_deref(),
            Some("3.14")
        );
    }
    ...
    fn atom_number_backward() {
        assert_eq!(
            tw().binding("atom_number(A, 42).", "A").as_deref(),
            Some("42")
        );
    }
    ...
    fn number_string_forward() {
        assert_eq!(
            tw().binding("number_string(42, S).", "S").as_deref(),
            Some("\"42\"")
        );
    }
    ...
    fn number_string_backward() {
        assert_eq!(
            tw().binding("number_string(N, \"42\").", "N").as_deref(),
            Some("42")
        );
    }
    ...
    fn term_string_atom() {
        assert_eq!(
            tw().binding("term_string(hello, S).", "S").as_deref(),
            Some("\"hello\"")
        );
    }
    ...
    fn term_string_number() {
        assert_eq!(
            tw().binding("term_string(42, S).", "S").as_deref(),
            Some("\"42\"")
        );
    }
    ...
    fn char_code_backward() {
        assert_eq!(
            tw().binding("char_code(Ch, 65).", "Ch").as_deref(),
            Some("A")
        );
    }
    ...
    fn atom_length_bind() {
        assert_eq!(
            tw().binding("atom_length(hello, N).", "N").as_deref(),
            Some("5")
        );
        assert_eq!(
            tw().binding("atom_length('', N).", "N").as_deref(),
            Some("0")
        );
    }
    ...
    fn string_length_bind() {
        assert_eq!(
            tw().binding("string_length(\"hello\", N).", "N").as_deref(),
            Some("5")
        );
    }
    ...
    fn atom_concat_forward() {
        assert_eq!(
            tw().binding("atom_concat(hello, world, X).", "X")
                .as_deref(),
            Some("helloworld")
        );
    }
    ...
    fn atom_concat_strip_prefix() {
        assert_eq!(
            tw().binding("atom_concat(hello, B, helloworld).", "B")
                .as_deref(),
            Some("world")
        );
    }
    ...
    fn atom_concat_strip_suffix() {
        assert_eq!(
            tw().binding("atom_concat(A, world, helloworld).", "A")
                .as_deref(),
            Some("hello")
        );
    }
    ...
    fn string_concat_forward() {
        assert_eq!(
            tw().binding("string_concat(\"foo\", \"bar\", X).", "X")
                .as_deref(),
            Some("\"foobar\""),
        );
    }
    ...
    fn upcase_atom() {
        assert_eq!(
            tw().binding("upcase_atom(hello, U).", "U").as_deref(),
            Some("HELLO")
        );
    }
    ...
    fn downcase_atom() {
        assert_eq!(
            tw().binding("downcase_atom('HELLO', D).", "D").as_deref(),
            Some("hello")
        );
    }
    ...
    fn upcase_string() {
        assert_eq!(
            tw().binding("upcase_string(\"hello\", U).", "U").as_deref(),
            Some("\"HELLO\"")
        );
    }
    ...
    fn atom_chars_forward() {
        assert_eq!(
            tw().binding("atom_chars(hi, Cs).", "Cs").as_deref(),
            Some("[h,i]")
        );
    }
    ...
    fn atom_chars_backward() {
        assert_eq!(
            tw().binding("atom_chars(A, [h, i]).", "A").as_deref(),
            Some("hi")
        );
    }
    ...
    fn atom_chars_empty() {
        assert_eq!(
            tw().binding("atom_chars('', Cs).", "Cs").as_deref(),
            Some("[]")
        );
    }
    ...
    fn atom_codes_forward() {
        assert_eq!(
            tw().binding("atom_codes(hi, Cs).", "Cs").as_deref(),
            Some("[104,105]")
        );
    }
    ...
    fn atom_codes_backward() {
        assert_eq!(
            tw().binding("atom_codes(A, [104, 105]).", "A").as_deref(),
            Some("hi")
        );
    }
    ...
    fn string_chars_forward() {
        assert_eq!(
            tw().binding("string_chars(\"hi\", Cs).", "Cs").as_deref(),
            Some("[h,i]")
        );
    }
    ...
    fn string_to_atom() {
        assert_eq!(
            tw().binding("string_to_atom(\"hello\", A).", "A")
                .as_deref(),
            Some("hello")
        );
    }
    ...
    fn atomic_list_concat_test() {
        assert_eq!(
            tw().binding("atomic_list_concat([hello, ' ', world], R).", "R")
                .as_deref(),
            Some("hello world"),
        );
    }
