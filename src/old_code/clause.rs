...
impl Clause {
    ...
    pub fn match_goal(
        ...
        //TO DO match with clause type to handle behaviour
        let mut subs = match self.atoms.get(0).unwrap().unify(goal) {
            Some(v) => v,
            None => return None,
        };
        subs.unmapped_to_qu(self.unmapped_vars(&subs), id_counter);
        let mut ho_sub = None;
        if self.higher_order() {
            ho_sub = Some(subs.universal_quantification());
        }
        return Some((subs, ho_sub));
    }
        ...
        match clause_string.find('\\') {
            Some(i) => {
                for symbol in clause_string[(i + 1)..].split(',') {
                    aqvars.insert(
                        Term::EQVar(symbol.to_string()),
                        Term::AQVar(symbol.to_string()),
                    );
                }
